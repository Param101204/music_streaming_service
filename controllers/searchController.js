...
export const search = async(req, res) => {
    ...
    try {
        ...
        if(search.rows.length === 0 && artists.rows.length === 0) {
            res.render('search.ejs', {data: map, len: map.size})
        } else if(artists.rows.length === 0) { 
            for ( var i = 0 ; i < search.rows.length; i ++ ) {
                const curr = search.rows[i]
                ...
                if(artistsQuery.rows.length > 0) {
                    ...
                    var imageToSend = artistsQuery.rows[0].artists_images;
            ...
            res.render('search.ejs', {data: map, len: map.size, username: req.session.username, playlists: playlists.rows});
            // res.redirect('/search');
        } else if(search.rows.length === 0) {

            for (var i = 0 ; i < artists.rows.length ; i ++) {
                const curr_id = artists.rows[i].song_id
                const songs = await getSongsById(curr_id);
                for ( var j = 0 ; j < songs.rows.length; j ++ ) {
                    const curr = songs.rows[j]
                    const id = curr.song_id
                    const artistsQuery = await getArtistsByID(id);
                    // console.log(artistsQuery.rows)
                    if(artistsQuery.rows.length > 0) {
                        const artists_ = []
                        // const default_image = "https://img.freepik.com/free-vector/futuristic-gradient-profile-picture_742173-9236.jpg?t=st=1729619249~exp=1729622849~hmac=55667e6b0eeba43d3bf64403680e0b83acda9e20e4f86ec7b91d78cb56808cc4&w=740"
                        var imageToSend = artistsQuery.rows[0].artists_images;
            ...
            for ( var i = 0 ; i < search.rows.length; i ++ ) {
                ...
                if(artistsQuery.rows.length > 0) {
                    ...
                    imageToSend = artistsQuery.rows[0].artists_images;
                    for ( var k = 0 ; k < artistsQuery.rows.length ; k ++ ) {
                        artists_.push(artistsQuery.rows[k].song_artists)
                    }
                    map.set(id, {song_name : curr.track_name, album_name: curr.album_name, genre: curr.genre, spotify_code: curr.track_id, album_name: curr.album_name, duration: curr.duration, rating: curr.rating, artist: artists_, image: imageToSend})
