...
export const artistController = async(req, res) => {
    ...
    try {
        ...
        for ( var i = 0 ; i < artistSongs.rows.length; i ++ ) {
            const curr = artistSongs.rows[i]
        ...
        res.render('artists.ejs', {data: map, len: map.size, username: req.session.username, image: imageToSend, artistName: artistName, playlists: playlists.rows});
    }  catch(err) {
