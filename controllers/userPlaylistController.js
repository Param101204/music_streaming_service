...
export const renderUserPlaylist = async(req, res) => {
    ...
    try {
        ...
        if(playlist.rows.length === 0) {
            ...
            const song_ids = await getSongsFromUserPlaylist(playlist.rows[0].playlist_id, playlistName);
            const songs = []
            for (const song_id of song_ids.rows) {
                // console.log(song_id.song_id);
                const query = await getSongsById(song_id.song_id);
                songs.push(query.rows); 
            }
            ...
            res.render('userPlaylists.ejs', {playlists: playlists.rows, data: map, len: map.size, userId:req.session.user_id, username: req.session.username, playlistImage: default_image, playlistName: playlistName, countOfSongs: songs_.length, playlistID: playlist.rows[0].playlist_id, creatorId: playlist.rows[0].user_id/*, countOfSongs: playlist.rows[0].count_of_songs, /*duration: playlist.rows[0].duration*/});
            // console.log(playlist.rows[0].playlist_id) 
            console.log(playlist.rows[0].user_id) 
        }
    } catch (err) {
        console.log(err);
    }
